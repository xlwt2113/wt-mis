...
public class YcHistoryController extends BaseController<YcHistory> {
    ...
    protected String generateSearchSql(YcHistory ycHistory, HttpServletRequest request) {
        StringBuffer sql = new StringBuffer("select t1.*,t2.hub_location,DATE_FORMAT(t1.update_time,'%Y-%d-%m %H:%i') as update_time_str from fi_yc_history  as t1 left join fi_dev_hub as t2 on t1.hub_id = t2.id   where t1.del = 0 ");
