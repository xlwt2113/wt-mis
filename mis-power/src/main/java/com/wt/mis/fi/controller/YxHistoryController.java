...
import com.wt.mis.fi.entity.YxHistory;
import com.wt.mis.fi.repository.YxHistoryRepository;
...
@Slf4j
@Controller
@RequestMapping("/fi/yxhistory")
public class YxHistoryController extends BaseController<YxHistory> {

    @Autowired
    YxHistoryRepository yxHistoryRepository;

    @Override
    public BaseRepository<YxHistory, Long> repository() {
        return yxHistoryRepository;
    }
    ...
    @Override
    protected String generateSearchSql(YxHistory yxHistory, HttpServletRequest request) {
        StringBuffer sql = new StringBuffer("select t1.*,t2.hub_location,DATE_FORMAT(t1.update_time,'%Y-%m-%d %H:%i') as update_time_str from fi_yx_history as t1 left join fi_dev_hub as t2 on t1.hub_id = t2.id  where t1.del = 0 ");
